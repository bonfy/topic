...
def jwt_token_required(f):
    ...
    def decorated(*args, **kwargs):
        token = None
        api_token = 'api_token'
        if API_TOKEN in request.headers and request.headers[API_TOKEN] == api_token:
            return f(*args, **kwargs)
        ...
        try:
            ...
            # print('Exception: ', e)
            return jsonify({'status': 'error', 'result': 'Token is invalid'}), 403
        return f(*args, **kwargs)
    return decorated
...
def login():
    ...
    # expires 60 seconds
    response.set_cookie('token', token, expires=time.time()+ COOKIE_EXPIRE_DAYS*24*60*60)
    return response
